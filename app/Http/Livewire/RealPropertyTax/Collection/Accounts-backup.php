...
class Accounts extends Component
{
    ...
    public function mount()
    {
        ...
        // $this->percentageValue['avYearNew'] = date("Y", strtotime($this->input_date));
        // $this->percentageValue['avYearOld'] = $this->percentageValue['avYearNew'] - 1;
        $this->month_selected = strtolower(date("F", strtotime($this->input_date)));
        $this->percentageValue = $this->getPenaltyPercentage($this->month_selected);
        $this->search_input = '';
    ...
    public function verify_record($id)
    {
        ...
        $checkForNewAv = $this->account_data->assessed_values
            ->where('av_year_from',$this->percentageValue['avYearNew'])->count();
            ## CHECKING NEW AV
        if($checkForNewAv < 1){
        ...
        if($this->account_data->rtdp_status != 1){
            ...
            if (is_null($this->account_data->rtdp_payment_covered_to) || empty($this->account_data->rtdp_payment_covered_to)) {
                ...
                ## sample only
                // $new = $this->account_data->assessed_values
                //     ->join('rpt_percentages', 'rpt_assessed_values.year_to', '=', 'rpt_percentages.year');
                //     // ->get(['country.country_name', 'state.state_name', 'city.city_name']);
                // dump($this->account_data->assessed_values);
                // dump($new);
                // dd('ok');
    ...
    private function settingVariables(){
        ...
        if (empty($this->lastPaymentQuarter) || is_null($this->lastPaymentQuarter) || $this->lastPaymentQuarter == 'Q4') {
            $this->nextPaymentYear = $this->lastPaymentYear + 1;
        } else {
            $this->nextPaymentYear = $this->lastPaymentYear + 0;
        }
        $this->lastPaymentQuarter = $this->lastPaymentYear + 1;
        ...
        $this->UnpaidAmount = $this->account_data->assessed_values->where('av_year_from','<=',$this->nextPaymentYear)->where('av_year_to','>=',$this->percentageValue['avYearNew'])->sortBy('av_year_from');
        $this->assessedValueOld = $this->account_data->assessed_values->where('av_year_from','=',$this->percentageValue['avYearOld'])->where('av_year_to','=',$this->percentageValue['avYearOld'])->first();
        $this->assessedValueNew = $this->account_data->assessed_values->where('av_year_from','=',$this->percentageValue['avYearNew'])->where('av_year_to','=',$this->percentageValue['avYearNew'])->first();
        // dump('lastPaymentYear ='.$this->lastPaymentYear);
        // dump('lastPaymentQuarter ='.$this->lastPaymentQuarter);
        // dump('nextPaymentYear ='.$this->nextPaymentYear);
        // dump('UnpaidAmount ='.$this->UnpaidAmount);
        // dump('assessedValueOld ='.$this->assessedValueOld);
        // dd('assessedValueNew ='.$this->assessedValueNew);
    ...
    private function compute_unpaid_amount()
    {
        ...
        $result = [];
        if ($this->nextPaymentYear == $this->percentageValue['avYearNew']) {
            $unpaid = [];
            $result = $this->newAvLoop($unpaid);
        } elseif($this->nextPaymentYear == $this->percentageValue['avYearOld']){
            $unpaid = [];
        ...
        foreach($data as $com){
            ...
            foreach($brackets as $b){
                if($b->year_from <= $com['from'] && $b->year_to >= $com['to']) {
                    ...
                    if($com['from'] == $this->percentageValue['avYearNew'] && $com['to'] == $this->percentageValue['avYearNew']){
                        switch ($com['quarter_value']) {
                            case 0.25:$newComputation[$c]['bracket']=$this->percentageValue['avYearNew']." Q1";
                                break;
                            case 0.50:$newComputation[$c]['bracket']=$this->percentageValue['avYearNew']." Q2";
                                break;
                            case 0.75:$newComputation[$c]['bracket']=$this->percentageValue['avYearNew']." Q3";
                                break;
                            default:$newComputation[$c]['bracket']=$this->percentageValue['avYearNew']." Q4";
                                break;
                        }
                    }else{
                        switch ($com['quarter_value']) {
    ...
    //  }
      ...
      private function regularAvLoop($unpaidQuarter){
        $quarterArray = $unpaidQuarter ?? [];
        $counter = count($quarterArray) ?? 0;
        $month = date("F", strtotime($this->input_date));
        // dd($month);
        // $quarter = round(ceil(($month/3)));

        $countDiff =  $this->percentageValue['avYearOld'] - $this->nextPaymentYear;
     ...
     private function oldAvLoop35($unpaid){
        ...
        $quarter = round(ceil(($month/3)));
        $penalty = RptPercentage::select('base')->where('from','<=',($this->percentageValue['avYearOld'] - 1))
        ->where('to','>=',($this->percentageValue['avYearOld'] - 1))->first();

        for ($x = 1; $x <= 1; $x++) {
            $quarterArray[$counter]['count'] = $counter;
            $quarterArray[$counter]['type'] = 'INC35';
            $quarterArray[$counter]['from'] = $this->percentageValue['avYearOld'];
            $quarterArray[$counter]['to'] = $this->percentageValue['avYearOld'];
            ...
            $quarterArray[$counter]['percentage'] = ($this->percetageValue['baseAvOld']+($month * 2)-2)/100;;
      ...
      private function oldAvLoop($unpaid){
        ...
        for ($x = $num; $x <= 4; $x++) {
            $quarterArray[$counter]['count'] = $counter;
            $quarterArray[$counter]['type'] = 'Q'.$this->percentageValue['avYearOld'];
            $quarterArray[$counter]['from'] = $this->percentageValue['avYearOld'];
            $quarterArray[$counter]['to'] = $this->percentageValue['avYearOld'];
            $quarterArray[$counter]['quarter_value'] = 0.25 * $x;
     ...
     private function newAvLoop($oldAv){
        ...
        for ($x = $num; $x <= 4; $x++) {
            $quarterArray[$counter]['count'] = $counter;
            $quarterArray[$counter]['type'] = 'Q'.$x.'('.$this->percentageValue['avYearNew'].')';
            $quarterArray[$counter]['from'] = $this->percentageValue['avYearNew'];
            $quarterArray[$counter]['to'] = $this->percentageValue['avYearNew'];
            $quarterArray[$counter]['quarter_value'] = 0.25*$x;
            $quarterArray[$counter]['quarter_label'] = 'Quarter '.$x;
            $quarterArray[$counter]['label'] = $quarterArray[$counter]['from'].' Q'.$x;
            $quarterArray[$counter]['year_no'] = 'Q'.$x;
            $quarterArray[$counter]['percentage'] = ($x < $quarter ? ($this->percetageValue['baseAvOld']+($month * 2)-2)/100 : $this->percentageValue['baseAvNew']/100);
    ...
    public function payOption2($data)
    {
        ...
        if (!is_null($data) || !empty($data)) {
            ...
            foreach ($data->groupBy('type') as $key => $value) {
                ...
                if ($value->first()['from'] == $this->percentageValue['avYearOld']) {
                ...
                } elseif($value->first()['from'] == $this->percentageValue['avYearNew']) {
                    // dump($value);
                    $someArray[$count]['quarter_label'] = $value->first()['label'];
                    $someArray[$count]['quarter_value'] = $value->first()['quarter_value'];
                } elseif($value->first()['from'] < $this->percentageValue['avYearOld']) {
