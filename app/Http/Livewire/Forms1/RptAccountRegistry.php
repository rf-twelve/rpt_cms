...
use App\Models\RptFormula_penalty;
use Livewire\Component;

class RptAccountRegistry extends Component
{
    ...
    {
        $this->updateOn = true;
        $this->dispatchBrowserEvent('rptRegistryOpen');
        $this->setFields($data);
        $this->findAV($data);
    ...
    public function addRecordEvent()
    {
        $this->emitSelf('refreshForm');
        $this->updateOn = false;
        $this->dispatchBrowserEvent('openRegisterForm');
        $this->reset();
    }
    ...
    {
        return view('livewire.forms.rpt-account-registry');
    }
}
